...
static void load_mmo(int msgcat)
{
	...
	int *mmo_index_buf = NULL;
	const char mmo_data_ptr_size = sizeof (int);
	int str_size;
	int size;
	int i;
	...
	{
		printf("mmo_config[msgcat].filename %s\n", mmo_config[msgcat].filename);
	}
	...
	mmo_index_buf = global_alloc_array(int, size);
	if (!mmo_index_buf)
		goto mmo_readerr;
	...
	{
		p->mmo_index[i].uid = (const char *)p->mmo_str + mmo_index_buf[i * mmo_data_ptr_size];
		p->mmo_index[i].ustr = (const char *)p->mmo_str + mmo_index_buf[i * mmo_data_ptr_size + 1];
		p->mmo_index[i].wid = (const char *)p->mmo_str + mmo_index_buf[i * mmo_data_ptr_size + 2];
		p->mmo_index[i].wstr = (const char *)p->mmo_str + mmo_index_buf[i * mmo_data_ptr_size + 3];
	}
